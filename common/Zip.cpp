...
bool ZipArchive::Open(const char* path, int flags, Error* error)
{
	pxAssert(!m_zip);
...
bool ZipArchive::WriteChangesAndClose(Error* error)
{
...
std::optional<ZipFileIndex> ZipArchive::LocateFile(const char* name, zip_flags_t flags, Error* error)
{
	...
	return static_cast<ZipFileIndex>(index);
}

std::optional<zip_stat_t> ZipArchive::StatFile(ZipFileIndex index, zip_flags_t flags, Error* error)
{
	zip_stat_t stat = {};
...
std::optional<std::string> ZipArchive::ReadTextFile(ZipFileIndex index, zip_flags_t flags, Error* error)
{
	ZipFile file;
...
std::optional<std::vector<u8>> ZipArchive::ReadBinaryFile(ZipFileIndex index, zip_flags_t flags, Error* error)
{
	ZipFile file;
	if (!file.Open(*this, index, flags, error))
		return std::nullopt;
...
std::optional<ZipFileIndex> ZipArchive::AddFile(
...
std::optional<ZipFileIndex> ZipArchive::AddFileFromSource(
...
bool ZipArchive::SetFileCompression(ZipFileIndex index, s32 comp, u32 comp_flags, Error* error)
{
...
ZipFile& ZipFile::operator=(ZipFile&& rhs)
{
	if (m_file)
		Close();
...
bool ZipFile::Open(ZipArchive& archive, ZipFileIndex index, zip_flags_t flags, Error* error)
{
	pxAssert(!m_file);
...
bool ZipFile::Read(void* buffer, u64 nbytes, Error* error)
{
	pxAssert(m_file);
...
bool ZipFile::Seek(s64 offset, int whence, Error* error)
{
	pxAssert(m_file);
...
std::optional<u64> ZipFile::Tell(Error* error)
{
	pxAssert(m_file);
...
std::optional<u64> ZipFile::Size(Error* error)
{
	pxAssert(m_file);
...
std::optional<std::string> ZipFile::ReadText(Error* error)
{
	pxAssert(m_file);
...
std::optional<std::vector<u8>> ZipFile::ReadBinary(Error* error)
{
	pxAssert(m_file);
...
bool ZipSource::CreateBuffer(const void* data, u64 length, int freep, Error* error)
{
	pxAssert(!m_source);
