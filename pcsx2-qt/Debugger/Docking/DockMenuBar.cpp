	...
	: QWidget(parent)
{
	...
	connect(m_layout_switcher, &QTabBar::customContextMenuRequested, this, &DockMenuBar::openContextMenu);
...
void DockMenuBar::openContextMenu(QPoint pos)
{
	int current_tab = m_layout_switcher->tabAt(pos);
	if (current_tab >= m_plus_tab_index)
		return;

	DockLayout::Index layout_index = static_cast<DockLayout::Index>(m_layout_switcher->tabAt(pos));

	QMenu* menu = new QMenu(m_layout_switcher);
	...
	bool reset_action_enabled = false;
	layoutCanBeReset(layout_index, &reset_action_enabled);

	QAction* reset_action = menu->addAction(tr("Reset Layout"));
	reset_action->setEnabled(reset_action_enabled);
	...
	menu->popup(m_layout_switcher->mapToGlobal(pos));
}
