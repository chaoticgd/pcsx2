...
void DockManager::loadLayouts()
{
	...
	{
		DockLayout::LoadResult result = DockLayout::FAILED;
	...
	// Make sure the layouts remain in the order they were in previously.
	...
	{
		if (layout_pointers[i] != &m_layouts[i])
			order_changed = true;
...
void DockManager::createWindowsMenu(QMenu* menu)
{
	...
	QAction* reset_default_layouts_action = new QAction(tr("Reset Default Layouts"), menu);
	connect(reset_default_layouts_action, &QAction::triggered, [this]() {
		QMessageBox::StandardButton result = QMessageBox::question(
	...
	menu->addAction(reset_default_layouts_action);
