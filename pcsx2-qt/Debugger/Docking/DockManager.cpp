...
QWidget* DockManager::createMenuBar(QWidget* original_menu_bar)
{
	...
	connect(m_menu_bar, &DockMenuBar::lockButtonToggled, this, [this](bool locked) {
		setLayoutLocked(locked, true);
	});
	connect(m_menu_bar, &DockMenuBar::editLayoutClicked, this, &DockManager::editLayoutClicked);
	connect(m_menu_bar, &DockMenuBar::resetLayoutClicked, this, &DockManager::resetLayoutClicked);
	connect(m_menu_bar, &DockMenuBar::deleteLayoutClicked, this, &DockManager::deleteLayoutClicked);
	connect(m_menu_bar, &DockMenuBar::layoutCanBeReset, this, [this](DockLayout::Index layout_index) {
		if (layout_index >= m_layouts.size())
			return false;

		DockLayout& layout = m_layouts[layout_index];
		return layout.canReset();
	});
...
void DockManager::setLayoutLocked(bool locked, bool write_back)
{
