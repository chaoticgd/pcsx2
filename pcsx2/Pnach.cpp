...
Pnach::PatchLineType Pnach::PatchLine::Type() const
{
	if (std::holds_alternative<Patch>(m_data))
		return PatchLineType::PATCH;
	else if (std::holds_alternative<DynamicPatch>(m_data))
		return PatchLineType::DPATCH;
	else if (std::holds_alternative<GSAspectRatio>(m_data))
		return PatchLineType::GSASPECTRATIO;
	else if (std::holds_alternative<GSInterlaceMode>(m_data))
		return PatchLineType::GSINTERLACEMODE;
	else
		return std::get<StringLine>(m_data).type;
}

Pnach::Patch& Pnach::PatchLine::GetPatch()
{
...
const Pnach::Patch& Pnach::PatchLine::GetPatch() const
{
...
void Pnach::PatchLine::SetPatch(Patch patch)
{
	m_data.emplace<Patch>(std::move(patch));
	ResetFormatting();
}

Pnach::DynamicPatch& Pnach::PatchLine::GetDynamicPatch()
{
...
const Pnach::DynamicPatch& Pnach::PatchLine::GetDynamicPatch() const
{
...
void Pnach::PatchLine::SetDynamicPatch(DynamicPatch dynamic_patch)
{
	m_data.emplace<DynamicPatch>(std::move(dynamic_patch));
	ResetFormatting();
}

Pnach::GSAspectRatio Pnach::PatchLine::GetGSAspectRatio() const
{
...
void Pnach::PatchLine::SetGSAspectRatio(GSAspectRatio aspect_ratio)
{
	m_data = aspect_ratio;
	ResetFormatting();
}

GSInterlaceMode Pnach::PatchLine::GetGSInterlaceMode() const
{
...
void Pnach::PatchLine::SetGSInterlaceMode(GSInterlaceMode interlace_mode)
{
	m_data = interlace_mode;
	ResetFormatting();
}

std::string_view Pnach::PatchLine::GetString() const
{
	const StringLine* data = std::get_if<StringLine>(&m_data);
	pxAssert(data);
	return std::string_view(data->string.get(), data->string_size);
}

void Pnach::PatchLine::SetString(PatchLineType type, std::string_view string, bool reset_formatting)
{
	pxAssert(type != PatchLineType::PATCH);
	pxAssert(type != PatchLineType::DPATCH);
	pxAssert(type != PatchLineType::GSASPECTRATIO);
	pxAssert(type != PatchLineType::GSINTERLACEMODE);

	StringLine& data = m_data.emplace<StringLine>();
	data.type = type;
...
void Pnach::PatchLine::SetSpacer()
{
	m_data = StringLine{PatchLineType::SPACER};
}

std::string_view Pnach::PatchLine::EndOfLineComment() const
{
	return std::string_view(m_end_of_line_comment.get(), m_end_of_line_comment_size);
}

void Pnach::PatchLine::SetEndOfLineComment(std::string_view comment, bool reset_formatting)
{
...
void Pnach::PatchLine::RemoveEndOfLineComment()
{
	m_end_of_line_comment.reset(nullptr);
	ResetFormatting();
}

void Pnach::PatchLine::ResetFormatting()
{
...
Pnach::PatchLine Pnach::PatchLine::FromString(std::string_view input, Error* error)
{
	PatchLine line;
	...
	if (!assignment.empty() && !line.ParseAssignment(assignment, error))
		line.SetString(PatchLineType::INVALID, assignment, false);
	...
	line.m_spaces_at_start_of_line = static_cast<u8>(spaces_at_start_of_line);
	...
	if (comment_delimiter_pos != std::string_view::npos)
	{
		...
		line.SetEndOfLineComment(comment, false);

		const size_t spaces_before_end_of_line_comment_delimiter = std::min(
			static_cast<size_t>(raw_assignment.end() - assignment.end()),
			static_cast<size_t>(std::numeric_limits<u8>::max()));
		line.m_spaces_before_end_of_line_comment_delimiter = static_cast<u8>(spaces_before_end_of_line_comment_delimiter);

		const size_t spaces_after_end_of_line_comment_delimiter = std::min(
			static_cast<size_t>(comment.begin() - raw_comment.begin()),
			static_cast<size_t>(std::numeric_limits<u8>::max()));
		line.m_spaces_after_end_of_line_comment_delimiter = static_cast<u8>(spaces_after_end_of_line_comment_delimiter);
	}

	return line;
}

bool Pnach::PatchLine::ParseAssignment(std::string_view assignment, Error* error)
{
	...
	{
		SetString(PatchLineType::AUTHOR, value, false);
	}
	else if (key == "comment")
	{
		SetString(PatchLineType::COMMENT, value, false);
	}
	else if (key == "description")
	{
		SetString(PatchLineType::DESCRIPTION, value, false);
	}
	else if (key == "gametitle")
	{
		SetString(PatchLineType::GAMETITLE, value, false);
	}
	else
	{
		return false;
...
std::string Pnach::PatchLine::ToString() const
{
	std::string result;
	...
	{
		const StringLine& line = std::get<StringLine>(m_data);

		switch (line.type)
		{
			case PatchLineType::AUTHOR:
			{
				result += "author";
				AppendAssignmentOperator(result);
				break;
			}
			case PatchLineType::COMMENT:
			{
				result += "comment";
				AppendAssignmentOperator(result);
				break;
			}
			case PatchLineType::DESCRIPTION:
			{
				result += "description";
				AppendAssignmentOperator(result);
				break;
			}
			case PatchLineType::GAMETITLE:
			{
				result += "gametitle";
				AppendAssignmentOperator(result);
				break;
		...
		result += std::string_view(line.string.get(), line.string_size);
	}
...
void Pnach::PatchLine::AppendAssignmentOperator(std::string& string) const
{
